...
# First, extract only the PCA columns using select and starts_with():
...
# change the  plot below so that the color is determined by the 'cluster_kmeans'
# column
...
# The labels in the clustering match the rownames in the input.
# And we use that to add the lineage_1 info.
...
# but if you have the time, go through it and understand each line.
ggplot() +
...
# Cut at height 40 (few, large clusters):
...
# Bonus challenge: Could you add the same cutlines to the ggplot version in 
# section 7b?
