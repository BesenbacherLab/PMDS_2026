...
# We can leave out the names of the parameters as long as we give them in the 
# right order:
ggplot(mpg_data, aes(x = displ, y = hwy)) +
  geom_point()


# EXERCISE 1: Now you try! Replace the '?' to plot 'cty' (city miles per gallon) on the y-axis
# (Hint: look at the example above)
ggplot(data = mpg_data, mapping = aes(x = displ, y = ?)) +
  geom_point()
...
ggplot(data = mpg_data, mapping = aes(x = displ, y = hwy, size = ?)) + 
  geom_point()
...
ggplot(data = mpg_data, mapping = aes(x = hwy)) + 
  geom_histogram()
...
# EXERCISE 7: Set color of the points to "red" and shape to "+"
# (note: these go OUTSIDE aes)
...
# If time allows, continue with the exercises in R for data science chapter 2: 
