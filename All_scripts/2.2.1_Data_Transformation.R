...
# EXERCISE G: Create a new variable showing the ratio of systolic(sbp) to diastolic(dbp) blood pressure
# Replace the ? with the calculation.
chd_data |>
...
# ifelse() can be used to create categorical variables based on conditions.

# Example: Create a categorical variable for high BMI og low BMI:
chd_data |>
  mutate(bmi_category = ifelse(bmi > 30, "HIGH", "LOW")) 

# If we want to create a logical variable (TRUE/FALSE) we can do that directly:
...
# Hint: First, modify the summary to include a count

# EXERCISE Q: Remove points based on fewer than 10 observations from the plot
