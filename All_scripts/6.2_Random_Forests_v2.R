...
rf_model <- 
  ...
  set_engine("ranger", 
    seed = 63233,             # Set seed for reproducibility
    importance = "impurity")  # Calculate variable importance based on impurity reduction (for Vip plots)

# make recipe, where we exlude id and followup (not predictors):
rf_recipe <- 
  recipe(chdfate ~ ., data = chd_train) %>%
  step_rm(id, followup)

# combine different parts of the model into a workflow:
rf_workflow <- 
  workflow() %>%
